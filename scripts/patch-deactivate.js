...
async function patchDeactivate() {
  try {
    ...
    if (extraDelhi) {
      console.log(`\nStep 3: PATCH deactivating DELETED_EXTRA_DELHI_WARRIORS (ID: ${extraDelhi.id})...`);
      ...
      if (updateResponse.status === 200 || updateResponse.status === 201) {
        console.log('✓ DELETED_EXTRA_DELHI_WARRIORS PATCH deactivated');
      } else {
        console.log('✗ Failed to PATCH deactivate');
      }
    } else {
